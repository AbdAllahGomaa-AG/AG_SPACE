...
export class NutritionAnalysisService {
  ...
  async analyzeMeal(request: AnalyzeMealRequest): Promise<AnalyzeMealResponse> {
    const logId = this.logger.start('NutritionAnalysisService', 'POST', `${this.functionsUrl}/analyze-meal`);

    const session = this.authService.session();
    if (!session) {
      this.logger.skip(logId, 'No active session — authService.session() is null');
    ...
    if (!request.textDescription && !request.imageBase64) {
      this.logger.skip(logId, 'No text or image provided');
    ...
    try {
      const response = await firstValueFrom(
        this.http.post<AnalyzeMealResponse>(
          ...
          retry(MAX_RETRIES),
          catchError((err: HttpErrorResponse) => this.handleHttpError(err, logId))
        )
      );
      ...
      this.logger.fail(logId, msg);
      console.error('[NutritionAnalysisService] analyzeMeal failed:', msg);
      return { success: false, error: msg };
    }
  }

  private handleHttpError(error: HttpErrorResponse, logId: string): Observable<never> {
    if (error.status === 0) {
    ...
    if (serverMsg) {
      this.logger.fail(logId, `Server error: ${serverMsg}`);
      return throwError(() => new Error(serverMsg));
    }

    if (error.status >= 500) {
      this.logger.fail(logId, `Server error (${error.status})`);
      return throwError(() => new Error('The analysis service encountered an error. Please try again later.'));
    }

    this.logger.fail(logId, `HTTP ${error.status}: ${error.message}`);
